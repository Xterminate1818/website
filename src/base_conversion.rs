...
fn ClearAllButton() -> impl IntoView {
  ...
  view! {
    <button class="secondary" on:click=move|_| write_hist(vec![])> "Clear history" </button>
  }
}
...
pub fn SizeButton(
  ...
  view! {
    <td role="button" on:click=set_size class="secondary" class:contrast=is_size>
      {name}
    </td>
  }
}
  ...
  F: Fn(GenericNum) + Copy + 'static,
{
  ...
  let bit_entry = move |n: usize| {
    view! {
      <td role="button" class = "secondary" class:contrast=move || get_bit(n) on:click=move|_| flip_bit(n)>
